...
class Familia:
    ...
    def comprar(self, imovel): #criação de função de compra calculado a partir e 20% do valor do imovel em comparação a poupança da familia
        #a decisao de compra avalia a pouca, se esta tem ao menos 20% do valor de compra do imovel
        for item in imovel:
            if item.valor * 0.2 <= self.poupanca:
                return f'Imóvel tipo {item.tipo} compra no valor de R${item.valor}.' #ocodigo passa pelas 3 opcoes de imovel(A,B,C)

        return 'Esta família não tem saldo suficiente na poupança para comprar nenhuma das opções de imóveis.'
